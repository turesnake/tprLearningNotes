...
"bidirectional reflectance distribution function"
公式：
	intensity = material( light_in, eye_out );
	---
	也就是，根据入角 和 出角的角度，计算特定材质中的 折射率
	也就是：尽管此次折射，能量衰减为原来的多少
...
# ----------------------------------------------#
规定了程序是如何为顶点之间的面着色的
假设 一个平面 是由 三个顶点 组成的 三角形面

--1-- flat shading         -- 平面着色
	每个平面用同一个颜色，最简单的着色法

--2-- gouraud shading      -- 高洛德着色
	根据三个顶点的信息差异，用最简单的方式，计算出平面内每个像素点的颜色
	使用 二维插值 来实现
	---
	在渲染一个 中低模球形时，还是能看出三角形的面
	而且，当球旋转时，高光区会从一组面，跳到另一组面，
	显得很不真实

--3-- phong shading        -- 冯氏着色
	三种方法中最最接近真实效果，也是开销最大的。
	为面中的每一个像素，单独计算它的 法向量（通过插值的方式）


	---
	在渲染一个 中低模球形时，已经无法看见三角形的面了
	能表现出一个非常平滑的球形
