...
// ----------------------------------- // 
//     基于 Texture3D 版生成 随机数
//           目前存在问题
// ----------------------------------- // 
/*
//static const float noiseTexSize = 8.0;
TEXTURE3D ( _Noise3D );  SAMPLER ( sampler_Noise3D );
float _Noise3DSize;

// 目前这个 函数还不能正常运作
// param: pos_: 整数坐标 [-inf, +inf]
float3 hash_05( float3 pos_ ){
    float3 uvw = fmod( pos_, _Noise3DSize ) / _Noise3DSize;
    return SAMPLE_TEXTURE3D( _Noise3D, sampler_Noise3D, uvw ).xyz;
}
*/
...
float4 gradientNoise3D_with_normal( float3 pos_ ){
    ...
    return float4( v, normalize(d.xyz) ); 
}
